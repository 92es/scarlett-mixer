...
#define DRAGGING 1 // Drag/scrollbar selector behavior
...
#if DRAGGING // Drag/scrollbar selector behavior
	float drag_x, drag_y;
	bool dragging;
	bool scrolled; // Differentiate between click and scroll mode in mouseup()
#endif
...
#if DRAGGING // Drag/scrollbar selector behavior
static void robtk_select_drag_stop(RobTkSelect *d) {
	d->dragging = FALSE;
	d->scrolled = FALSE;
}
...
static RobWidget* robtk_select_mouseup(RobWidget* handle, RobTkBtnEvent *ev) {
	...
	// End with single click or drag/scroll behavior, not both
	if (d->dragging && d->scrolled) {

		robtk_select_drag_stop(d);
...
#define MIN_DRAG_THRESHOLD 2
		if (diff <= -MIN_DRAG_THRESHOLD || MIN_DRAG_THRESHOLD <= diff) {

//printf("robtk_select_mousemove( DRAGGING ) x %d, y %d, diff %f\n", ev->x, ev->y, diff);

			// Differentiate between click and scroll modes in mouseup()
			d->scrolled = TRUE; 
			d->drag_x = ev->x;
			d->drag_y = ev->y;

			int save_direction = ev->direction; // save
			ev->direction = (diff < 0.0 ? ROBTK_SCROLL_LEFT : ROBTK_SCROLL_RIGHT);
			robtk_select_scroll(handle, ev);
			ev->direction = save_direction;  //restore
		}
	}
#endif
